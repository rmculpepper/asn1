...
(define (rt1 type value)
  (define ber (asn1->bytes type value))
  (define der (asn1->bytes/DER type value))
  (vector type value ber der))

(define (rt type . values)
  (when #f (eprintf "* testing ~v\n" type))
  (for/list ([value values]) (rt1 type value)))

(define benchmark-rows
  (append
   (rt BOOLEAN #t #f)
...
(time
 (for ([i (in-range #e1e2)])
   (for ([row (in-list benchmark-rows)])
     (match-define (vector type value ber der) row)
     (bytes->asn1 ANY ber)
     (bytes->asn1 ANY der))))
